...
class CartController extends Controller
{
    ...
    public function store(Request $request)
    {
        //dd($request->all());
        $cart = $this->cart->findOrCreate(auth()->user()->id);
        $product = Product::with('variants.attributes.attribute', 'variants.attributes.value')->find($request->id);
        $cartproduct = $this->cartProduct->where('cart_id', $cart->id)
            ->where('product_id', $product->id)
            ->first();
        $variant = $product->has_variants == 1 ? $product->variants->where('id', $request->variant)->first() : null;
        $attributeText = '';

        if ($variant && $variant->attributes) {
            $attributeText = $variant->attributes->map(function ($attr) {
                return $attr->value->label;
            })->implode(' - ');
        }

        if (($product->quantity > 0 && $product->has_variants == 0) || ($product->has_variants == 1 && $variant->stock > 0)) {
            if ($cartproduct == null) {
                ...
                $this->cartProduct->create([
                    ...
                    'product_name' => $product->name . ($variant ? ' - ' . $attributeText : ''),
                    'product_price' => $variant ? $variant->price * $product->sale_percent :  $product->price * $product->sale_percent,
