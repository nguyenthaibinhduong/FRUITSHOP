   ...
   public function getProductDetail($id)
   {
      ...
      foreach ($product->variants as $variant) {
         foreach ($variant['attributes'] as $attri) {
            $name = $attri['attribute']['name'];
            $displayType = $attri['attribute']['display_type'];
            $valueId = $attri['value']['id'];

            // Đảm bảo không trùng giá trị
            if (!isset($groupedAttributes[$name]['values'][$valueId])) {
               $groupedAttributes[$name]['display_type'] = $displayType;
               $groupedAttributes[$name]['values'][$valueId] = $attri['value'];
            }
         }
      }
      $comments = $product->comments;
      if ($product != null) {
         return view('client.product-detail', compact('product', 'image', 'thumps', 'comments', 'groupedAttributes'));
      } else {
         return redirect()->route('404');
      }
   }
