    <script src="{{ secure_asset('client/js/jquery-3.3.1.min.js') }}"></script>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.3/dist/umd/popper.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.12.0/dist/sweetalert2.all.min.js"></script>
    <script src="{{ secure_asset('client/js/bootstrap.min.js') }}"></script>
    <script src="{{ secure_asset('client/js/jquery.nice-select.min.js') }}"></script>
    <script src="{{ secure_asset('client/js/jquery-ui.min.js') }}"></script>
    <script src="{{ secure_asset('client/js/jquery.slicknav.js') }}"></script>
    <script src="{{ secure_asset('client/js/mixitup.min.js') }}"></script>
    <script src="{{ secure_asset('client/js/owl.carousel.min.js') }}"></script>
    ...
    <script>
        $(document).ready(function() {
                    ...
                    success: function(res) {
                        if (res.length > 0) {
                            for (var cart of res) {
                                ...
                                html += '<tr>';
                                html += '<td><img width="40px" height="40px" src="' + urlImage +
                                    '" alt=""></td>';
                                html += '<td>' + cart.product_name.substring(0, 18) + '</td>';
                                html += '<td> x ' + cart.quantity + '</td>';
                                html += '<td class="subtotal font-weight-bold">' + (cart
                                    .product_price * cart.quantity).toString().replace(
                                    /\B(?=(\d{3})+(?!\d))/g, ".") + 'đ' + '</td>';
                                html += '</tr>';
                            }
            ...
            $('#button-comment').on('click', function(event) {
                ...
                if (user_id != "") {
                    $.ajax({
                        url: (option != 0) ? ("{{ url('api/update-comment') }}" + "/" + option) : (
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.length > 0) {
                            for (var pro of res) {
                                urlImage = pro.url;
                                urlPro = "{{ url('/product') }}" + '/' + pro.product_id;
                                //html+='<li class="list-group-item  ">';
                                html +=
                                    '<a class="list-group-item list-group-item-action d-flex p-2" href="' +
                                    urlPro + '">';
                                html += '<div class="d-flex align-items-center">';
                                html += ' <img width="40px" height="40px" src="' + urlImage +
                                    '" alt="">';
                            ...
                            html +=
                                '<a class="list-group-item list-group-item-action d-flex p-2" href="#"><i>Không có kết quả tìm kiếm</i><a>';
                            $('.resultSearch').html(html);
                        }
